...
$(document).ready(function(){
  ...
  // Chages the order of Partners photos
  if ($('#Lucas').css('display', 'inline-block')) {
    $('#Lucas').css('display', 'block');
    $('#Heather').insertBefore('#Lucas');
  } else {
    $('#Lucas').insertBefore('#Heather');
  }
});
    ...
    if (request.status === 200 && request.readyState === 4) {
      ...
      if (responseText.hits.length === 0) {
        ...
        for ( var i = 0; i < counter; i++ ) { // Dynamicaly creating the content
          ...
          activitiesItemBg.appendChild(activitiesPhoto);
          activitiesItemBg.appendChild(activitiesText);
